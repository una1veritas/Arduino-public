...
byte membuffer[1024];
SRAMBuffer sram(membuffer, 1024);
...
// [SPI or I2C Mode declaration], [gyro I2C address], [xm I2C address]
 ...
 ///////////////////////////////
 const byte DRDYG = 46;  // DRDYG tells us when gyro data is ready
 const byte INT1XM = 47; // INT1XM tells us when accel data is ready
 const byte INT2XM = 48; // INT2XM tells us when mag data is ready
...
//void pressure_measurement(bool gettemp = false);
// Sensirion SHT7x/1x
const byte SENSIRION_DATA = 4;
const byte SENSIRION_SCK = 3;
...
const int PIN_GPSONOFF = 5;
//const int PIN_GPS_PPS = 15;
GPS gps(GPS_SERIAL, PIN_GPSONOFF);
struct Satellite {
  unsigned long lastmillis;
  unsigned long period;
  
  Satellite() { period = 5000; }
} sat;
...
void setup() {
  ...
  MESSAGE << F("9DoF, ");
  MESSAGE.println(stat, HEX);

  rtc.begin();
  rtc.start();
  if ( rtc.update() ) {
    MESSAGE << F("ST M41T62 started. ");
    MESSAGE.print(rtc.time, HEX); 
    MESSAGE << NL;
    do {
      long tmp = rtc.time;
...
//  pinMode(PIN_GPS_PPS, INPUT);
  ...
  SPI.begin();
  sram.begin();
  MESSAGE << F("SPISRAM, ") << NL;
  while ( sram.available() )
    MESSAGE.print((char)sram.read());
  pinMode(SD_CS, OUTPUT);
  if ( SD.begin(SPI_CLOCK_DIV2) ) 
    MESSAGE << F("SD Card started.") << NL;
  MESSAGE.println();
...
void loop() {
  ...
  if ( lastmillis_LSM9DS0 + 125 <= millis() ) {
    ...
    if ( digitalRead(INT1XM) && dof9.readAccel() ) {
      LOGOUT << F(",ACCEL:");
      LOGOUT.print(dof9.accelX(), 4); 
      LOGOUT.print(',');
      LOGOUT.print(dof9.accelY(), 4); 
      LOGOUT.print(',');
      LOGOUT.print(dof9.accelZ(), 4);
    }
    if ( digitalRead(DRDYG) && dof9.readGyro() ) {
      LOGOUT << F(",GYRO:");
      LOGOUT.print(dof9.gyroX(), 4); 
      LOGOUT.print(',');
      LOGOUT.print(dof9.gyroY(), 4); 
      LOGOUT.print(',');
      LOGOUT.print(dof9.gyroZ(), 4);
    }
    LOGOUT << NL;
  }
  ...
  if ( sat.lastmillis + sat.period <= millis() ) {
    if ( gps.catchMessage(10) == 0x524d43 ) {
      ...
      if ( gps.readRMC() ) {
        LOGOUT << F(",GPSRMC:");
        LOGOUT.print(gps.date(), 0);
        LOGOUT << ',';
        LOGOUT.print(gps.UTC(),0);
        LOGOUT << ',';
        LOGOUT.print(gps.latitude(),4);
        LOGOUT << ','; 
        LOGOUT.print(gps.longitude(),4);
        LOGOUT << ',';
        LOGOUT.print(gps._knots, 4);
        LOGOUT << ',';
        LOGOUT.print(gps._degree, 4);
        LOGOUT<< NL;
  ...
  if ( weather.lastmillis + weather.period <= millis() ) {
    ...
    if ( weather.temp_ready && weather.humi_ready && weather.atpr_ready ) {
      weather.lastmillis = millis();
      weather.atpr100 = barop.pressure(); //getPressure(MODE_ULTRA);
      rtc_output();
      LOGOUT << F(",WTH:") << ((double)weather.atpr100/100) << ',' 
        << ((double)weather.temp100/100) << ',' 
        << ((double)weather.humi100/100);
      if (  digitalRead(INT2XM) && dof9.readMag() ) {
        LOGOUT << F(",MAG:");
        LOGOUT.print(dof9.magX(), 4);
        LOGOUT.print(',');
        LOGOUT.print(dof9.magY(), 4);
        LOGOUT.print(',');
        LOGOUT.print(dof9.magZ(), 4);
      }
      LOGOUT<< NL;
      weather.clear();
    } 
...
void rtc_output() {
  rtc.update();
  LOGOUT.print( rtc.date, HEX);
  LOGOUT << ',';
  LOGOUT.print(rtc.BCD2uint(rtc.time));
  LOGOUT << F(",");
  unsigned long t = (millis() - millis_offset) % 1000;
  LOGOUT.print(t/100 % 10);
  LOGOUT.print(t/10%10);
  LOGOUT.print(t%10);
}
