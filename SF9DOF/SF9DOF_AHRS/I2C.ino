...
/* ******************************************************* */
  ...
  #define WIRE_READ(x)  Wire.read(x)
#else
...
void Accel_Init()
{
  ...
  WIRE_WRITE(0x2D);  // power register
  WIRE_WRITE(0x08);  // measurement mode
  Wire.endTransmission();
  delay(5);
  Wire.beginTransmission(AccelAddress);
  WIRE_WRITE(0x31);  // Data format register
  WIRE_WRITE(0x08);  // set to full resolution
  Wire.endTransmission();
  delay(5);	
  // Because our main loop runs at 50Hz we adjust the output data rate to 50Hz (25Hz bandwidth)
  Wire.beginTransmission(AccelAddress);
  WIRE_WRITE(0x2C);  // Rate
  WIRE_WRITE(0x09);  // set to 50Hz, normal operation
  Wire.endTransmission();
  delay(5);
}
...
void Read_Accel()
{
  ...
  Wire.beginTransmission(AccelAddress); 
  WIRE_WRITE(0x32);        //sends address to read from
  ...
  { 
    buff[i] = WIRE_READ();  // receive one byte
    i++;
  }
  Wire.endTransmission(); //end transmission
...
void Compass_Init()
{
  ...
  WIRE_WRITE(0x02); 
  WIRE_WRITE((byte)0x00);   // Set continouos mode (default to 10Hz)
  Wire.endTransmission(); //end transmission
}
...
void Read_Compass()
{
  ...
  Wire.beginTransmission(CompassAddress); 
  WIRE_WRITE(0x03);        //sends address to read from
  Wire.endTransmission(); //end transmission
