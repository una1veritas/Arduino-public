...
class ITG3200 {
	...
	static const int ITG_ADDR = 0xD0>>1; //0xD0 if tied low, 0xD2 if tied high

	public:
		ITG3200(char i2c_address = ITG_ADDR);
		void begin(void);
//		char read(char register_addr, char * value);
		char read(char register_addr, char * value, int num = 1);
		...
		float getZ(void);
//		float getTemp(void);
		inline int16_t x() { return raw[0]; }
		inline int16_t y() { return raw[1]; }
		inline int16_t z() { return raw[2]; }
