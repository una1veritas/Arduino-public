...
class ADXL345 {
	...
	static const int ADXL_ADDR = 0xA6>>1;

	public:
		ADXL345(char i2c_address = ADXL_ADDR);
		char begin(void);
		...
		inline int16_t x() { return values[0]; }
		inline int16_t y() { return values[1]; }
		inline int16_t z() { return values[2]; }
	...
	private:
		...
		char read(char register_addr, char * value, int num);
};
