...
void setup() {
  ...
  if ( nfc.GetGeneralStatus() && (cnt = nfc.getCommandResponse(respbuff)) ) {
    printBytes(Serial, respbuff, cnt);
...
void loop() {
  ...
  *((word*) &payload[1]) = (word) 0x00fe;
  if ( nfc.InListPassiveTarget(1, NFC::CARDTYPE_GENERICPASSIVE_212K_F, payload, 5) ) {
    cnt = nfc.getCommandResponse(respbuff);
    if ( cnt)
      nfc.targetSet(NFC::CARDTYPE_FELICA_212K, respbuff+4, 8);
    ...
    if ( ver != 0xffff ) {
      ...
      if ( cnt != 0 ) {
        printBytes(Serial, respbuff, cnt*16);
