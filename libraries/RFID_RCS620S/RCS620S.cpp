...
uint16_t RCS620S::FeliCa_ReadWithoutEncryption(uint16_t serviceCode, uint16_t blknum,
	...
	memcpy(buf + 1, idm, 8);
	buf[9] = 0x01;      // 繧ｵ繝ｼ繝薙せ謨ｰ
	buf[10] = lowByte(serviceCode);
	buf[11] = highByte(serviceCode);
	buf[12] = 0x01;     // 繝悶Ο繝�け謨ｰ
...
uint16_t RCS620S::Mifare_Read(const uint8_t blkno, uint8_t * resbuf) {
	uint16_t result;

	resbuf[0] = MIFARE_CMD_READ;
	resbuf[1] = blkno;
	result = InDataExchange(resbuf, 2);
#ifdef DEBUG
	Serial.print("receive: ");
	Serial.printBytes(resbuf, result);
	Serial.println(".\n");
#endif
	if ( result >= 3 ) {
		memmove(resbuf, resbuf+3, result - 3);
		return result - 3;
	}
	return result;
}

uint16_t RCS620S::Mifare_BlockAuth(const uint8_t blkno, const uint8_t keyAB[8]) {
  uint8_t buf[32];
  uint16_t reslen;
  
  if ( keyAB[0] == 'B' || keyAB[0] == 0x0B || keyAB[0] == 0xBB  ) {
	  buf[0] = MIFARE_CMD_AUTH_KEYB;
  } else {
	  buf[0] = MIFARE_CMD_AUTH_KEYA;
  }
  buf[1] = blkno;
  memcpy(buf+2, keyAB+1, 6);
...
邵ｲ�ｽ陞滂ｽｧ鬩･蠑ｱ縲堤ｸｺ蜷ｶ�ｽ�ｽ

闔会ｽ･闕ｳ荵昶�ｲRCS620S.cpp邵ｺ�ｫ騾ｶ�ｸ陟冶侭笘�ｹｧ遏ｩﾎ夊崕�ｽ縲堤ｸｺ蜷ｶ�ｽ繧托ｽｼ莠･�ｮ貊�怙邵ｺ�ｯmain鬩幢ｽｨ邵ｺ�ｫ闕ｳ�ｽ驍ｱ蛛ｵ竊鍋ｹｧ�ｳ郢晢ｽｼ郢晢ｿｽ縺�ｹ晢ｽｳ郢ｧ�ｰ邵ｺ蜉ｱ窶ｻ邵ｺ�ｽ竏ｪ邵ｺ蜻ｻ�ｼ�ｽ
...
/* readNFC command */     
//邵ｲ�ｽ郢ｧ�ｫ郢晢ｽｼ郢晏ｳｨ�ｽ闕ｳ�ｭ郢ｧ螳夲ｽｪ�ｭ邵ｺ�ｿ髴趣ｽｼ郢ｧ�ｽ
...
//		Serial.print(responseLen);
		if (responseLen == 11) {		// FCF郢ｧ�ｫ郢晢ｽｼ郢晏ｳｨ縲堤ｸｺ�ｪ邵ｺ莉｣�檎ｸｺ�ｰIDm郢ｧ蛛ｵ笳守ｸｺ�ｮ邵ｺ�ｾ邵ｺ�ｾ髴第鱒笘�
			...
			Outbuf[16] = 0x00;
		} else if (memcmp(buf + 44, "\x34\x30\x30\x31\x30\x30\x37\x31", 8) == 0) {						//邵ｲ�ｽFeliCaFCF邵ｺ�ｽ邵ｺ�｣邵ｺ貅ｷ�ｽ�ｴ陷ｷ�ｽ
			//闕ｵ譎擾ｽｷ�･陞滂ｽｧ邵ｺ�ｮ郢ｧ�ｫ郢晢ｽｼ郢晏ｳｨ笆｡邵ｺ�｣邵ｺ貅假ｽ�
/*			j = 0;
			for(i = 12;i < 22; i++) { 	//陝�ｽｦ驍��ｽ 髢ｨ�ｷ陷ｩ�｡騾｡�ｪ陷ｿ�ｷ
				Outbuf[j] = buf[i];
				j++;
			}
			Outbuf[10] = buf[26];  // 陷�蜥ｲ蛹ｱ髯ｦ蠕後Ψ郢晢ｽｩ郢ｧ�ｰ
			...
			// FCF邵ｺ�ｧ郢ｧ繧�ｽｹ譎擾ｽｷ�･陞滂ｽｧ邵ｺ�ｮ郢ｧ�ｫ郢晢ｽｼ郢晏ｳｨ縲堤ｸｺ�ｪ邵ｺ莉｣�檎ｸｺ�ｰIDm郢ｧ螳夲ｽｿ譁絶�
			Outbuf[0] = (((IDm[0]>>4) & 0x0F) + 0x30 );
			Outbuf[1] = ((IDm[0] & 0x0F) + 0x30 );
		 	if (Outbuf[0] > 0x39 ) Outbuf[0] = Outbuf[0] + 0x07;
			if (Outbuf[1] > 0x39 ) Outbuf[1] = Outbuf[1] + 0x07;
...
/*			for (i=0; i<10; i++) {	//example S828234212
	...
	xQueueSendToBack(keyQueue, Outbuf, 0); //TASK4邵ｺ�ｸ
	return 1;
}
...
--
陞滂ｽｧ鬩･謇区ぅ闕ｵ�ｽ <ohno@tech-i.kyutech.ac.jp>
