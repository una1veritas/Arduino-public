...
void ST7565::begin(uint8_t cont) {
  init(cont);
}

void ST7565::init(byte cont) {
  // set pin directions
  pinMode(pin_cs, OUTPUT);
  digitalWrite(pin_cs, LOW);
  pinMode(pin_rs, OUTPUT);

  command(CMD_DISPLAY_OFF); // Display = OFF
  command(CMD_SET_ADC_NORMAL);  // ADC = normal
  command(CMD_SET_COM_REVERSE);  // Common output = reverse
  command(CMD_SET_BIAS_7); // LCD bias select

  command(CMD_SET_POWER_CONTROL1);// power control 1, 2c
  delay(2);
  command(CMD_SET_POWER_CONTROL2);// power control 2, 2e
  delay(2);
  command(CMD_SET_POWER_CONTROL3);// power control 3, 2f

  // contrast setting

  command(0x23);  // V0 voltage resistor ratio set
  command(CMD_SET_VOLUME_FIRST); // electronic volume mode set
  command(cont & 0x3f);  // electronic volume value set
...
void ST7565::deselect() {
	digitalWrite(pin_cs, HIGH);
}

void ST7565::send(byte mode, byte val) {
	select();
...
void ST7565::contrast(uint8_t val) {
  ...
  */
  ...
  for(p = 0; p < 8; p++) {
    ...
    */
    ...
    command(CMD_SET_COLUMN_LOWER | ((col+ST7565_STARTBYTES) & 0xf));
    command(CMD_SET_COLUMN_UPPER | (((col+ST7565_STARTBYTES) >> 4) & 0x0F));
    command(CMD_RMW);
...
void ST7565::clear(void) {
  ...
  for(p = 0; p < 8; p++) {
    ...
    */
    ...
    command(CMD_SET_PAGE | p);
    for(c = 0; c < 129; c++) {
      //uart_putw_dec(c);
      //uart_putchar(' ');
      command(CMD_SET_COLUMN_LOWER | (c & 0xf));
      command(CMD_SET_COLUMN_UPPER | ((c >> 4) & 0xf));
      data(0x0);
    }     
  }
}
