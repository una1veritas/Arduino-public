...
CameraC328R::CameraC328R(Stream & port) : _serialPort(port)
{
  _packageSize = DEFAULT_PACKAGE_SIZE;
}

/*
CameraC328R::CameraC328R(SoftwareSerial & softSerial) : _serialPort((Stream&) softSerial)
{
	serial_class = SOFTWARESERIAL;
  _packageSize = DEFAULT_PACKAGE_SIZE;
}

CameraC328R::CameraC328R(HardwareSerial & serial) : _serialPort((Stream&) serial)
{
	serial_class = HARDWARESERIAL;
  _packageSize = DEFAULT_PACKAGE_SIZE;
}
*/
/*
void CameraC328R::begin(long baud) {
	switch(serial_class) {
	case SOFTWARESERIAL:
		((SoftwareSerial&)_serialPort).begin(baud);
		break;
	case HARDWARESERIAL:
		((HardwareSerial&)_serialPort).begin(baud);
		break;
	}
}
*/
...
//
bool CameraC328R::printJPEGPicture(Stream & outstream, PictureType pictureType, uint16_t processDelay )
{
  uint16_t pictureSize = 0;
...
//      callback( pictureSize, packagePictureDataSize, packageCount, package + PACKAGE_DATA_START );
      ...
      outstream.write(package + PACKAGE_DATA_START, packagePictureDataSize);
      outstream.flush();
...
{
	_serialPort.write(_command, CMD_SIZE);
	/*
  uint8_t i;
...
bool CameraC328R::waitForResponse( uint32_t timeout, byte buffer[], uint16_t bufferLength )
{
  ...
  {
    while( /* serial_*/ _serialPort.available() > 0 )
    {
      buffer[byteCnt] = _serialPort.read(); //serial_read();
      byteCnt++;
